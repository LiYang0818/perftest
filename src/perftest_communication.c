		...
		struct rdma_cm_id	**cm_id_control)
{
	...
	if (user_param->use_rdma_cm) {
		...
		#endif
		comm->rdma_ctx->buff_size = user_param->cycle_buffer;
		if (*cm_channel_control == NULL || *cm_id_control == NULL) {
			if (create_rdma_resources(comm->rdma_ctx,comm->rdma_params)) {
				fprintf(stderr," Unable to create the resources needed by comm struct\n");
				goto free_mem;
			}
			*cm_channel_control = comm->rdma_ctx->cm_channel;
			*cm_id_control = comm->rdma_ctx->cm_id_control;
		}
		else {
			comm->rdma_ctx->cm_channel = *cm_channel_control;
			comm->rdma_ctx->cm_id_control = *cm_id_control;
		}
	}
